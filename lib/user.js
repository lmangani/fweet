...
User.delPosts = function(uid, start, count, callback) {
   var key = uid == -1 ? 'global:timeline' : 'uid:' + uid + ':posts';
   var multiAction = r.multi();
   // Trim latest post from Thing

   r.lrange(key, start, count, function(err,pids){
      if (err) return callback(err);
      r.mget(pids.map(function(pid){return 'post:'+pid}),function(err,rawPosts) {
         if (err) return callback(err);
         var posts = rawPosts.map(function(postString){return Post.fromString(postString)});
      ...
      var delposts = pids.map(function(post){ r.del('post:'+post); r.lrem('uid:'+uid+':posts',-1,post); r.lrem('global:timeline',-1, post);  });
   });

/*
   r.ltrim(key, start, count, function(err,pids){
      if (err) return callback(err);
      return callback(undefined, posts);
   });
...
User.delAllPosts = function(uid, callback) {
   var key = uid == -1 ? 'global:timeline' : 'uid:' + uid + ':posts';
   var multiAction = r.multi();
   r.lrange(key, 0, -1, function(err,pids){
      if (err) return callback(err);
      // Delete all posts from Thing
      r.mget(pids.map(function(pid){return 'post:'+pid}),function(err,rawPosts) {
         if (err) return callback(err);
         var posts = rawPosts.map(function(postString){return Post.fromString(postString)});
         return callback(undefined, posts);
...
User.setTokenWithData = function(token, data, ttl, callback) {
    ...
    r.set(token, JSON.stringify(userData), function(err, reply) {
        if (err) callback(err);
 
        if (reply) {
            r.expire(token, timeToLive, function(err, reply) {
                if (err) callback(err);
